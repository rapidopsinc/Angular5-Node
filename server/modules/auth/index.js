let MailOptions = require("../../utils/send-mail");
...
module.exports = class AuthController {
    constructor(app) {
        ...
        app.get('/validate-reset-password-token/:token', this.validateToken);
        app.put('/reset-forgot-password', this.resetForgotPasssword);
            ...
            .then((user) => {
                if (user != null) {
                    if (user.password === oldpassword) {
                        ...
                        res.sendError("Your current password does not match the entered password");
                    }
                }
                user.save()
                    .then((user) => {
...
/*
    doLogin(req, res) {
        let username = req.body.username;
        let password = req.body.password;
        let userCredential = global.config['adminCredential'];
        console.log('userCredential', userCredential);

        if (username === userCredential.userName && password === userCredential.password) {
                req.session.isLoggedIn = 'Y';
                req.session.userProfile = req.body;
                return res.send({status: "success", data: req.session.userProfile});
        } else {
            return res.send({status: "failure", message: "Invalid Login Details !"});
        }
